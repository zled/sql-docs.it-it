...
##  <a name="bkmk_view_log"></a> Dove si trovano i file di log del server di report?  
 ...
 I registri di traccia vengono creati quotidianamente, a partire dal primo evento che si verifica dopo la mezzanotte (ora locale), e ogni volta che il servizio viene riavviato. Il timestamp si basa su l'ora UTC (Coordinated Universal Time). Il file è in formato en-US Per impostazione predefinita, i log di traccia possono occupare uno spazio massimo di 32 MB e vengono eliminati dopo 14 giorni.  
...
##  <a name="bkmk_trace_configuration_settings"></a> Impostazioni di configurazione della traccia  
 Comportamento di log di traccia viene gestito nel file di configurazione **Reportingservicesservice.exe**. Il file di configurazione si trova nel seguente percorso:  
...
|**Prefix**|Specifica un valore generato che distingue ogni istanza del log dalle altre.|Per impostazione predefinita, ai nomi file dei log di traccia vengono aggiunti valori timestamp. Questo valore è impostato su "tid appdomain, tempo". Non modificare questa impostazione.|  
