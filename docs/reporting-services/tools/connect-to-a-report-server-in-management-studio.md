...
ms.topic: "article"
helpviewer_keywords: 
  - "server di report [Reporting Services], connessioni"
  - "connessioni [Reporting Services], server di report"
  - "registrazione di server di report"
  - "server di report [Reporting Services], registrazione"
...
### Per connettersi a un server di report in modalità nativa  
...
### Per connettersi a un server di report in modalità integrata SharePoint  
  
1.  Se Esplora oggetti non è già aperto, selezionarlo dal menu Visualizza.  
  
2.  Fare clic su per visualizzare l'elenco dei tipi di server, quindi selezionare **Reporting Services**.  
  
3.  Nella finestra di dialogo **Connetti al server**, immettere un URL a un sito di SharePoint. L'esempio seguente illustra la sintassi, ovvero http://\<server Web>/siti/\<sito>.  
...
### Per registrare un server di report  
...
## Sintassi della connessione e autorizzazioni  
