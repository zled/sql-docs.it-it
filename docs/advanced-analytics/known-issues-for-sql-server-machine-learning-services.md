...
Per SQL Server 2017, nell'aggiornamento cumulativo 5 a 7, è disponibile una regressione nel **rlauncher.config** file in cui il percorso del file directory temp include uno spazio. Regressione viene risolto entro CU8.

L'errore che verrà visualizzato quando si esegue lo script R include due messaggi seguenti:
...
Applicare CU8 quando diventa disponibile. In alternativa, è possibile ricreare **rlauncher.config** eseguendo **registerrext** con disinstallare/installazione in un prompt dei comandi con privilegi elevati. 
...
Nell'esempio seguente mostra i comandi con l'istanza predefinita "MSSQL14. Installato MSSQLSERVER"in" c:\Programmi\Microsoft SQL Server\":
...
## <a name="r-script-execution-issues"></a>Problemi dell'esecuzione di script R
...
### <a name="use-of-strings-as-factors-can-lead-to-performance-degradation"></a>L'uso di stringhe di esempio fattori possono comportare una riduzione delle prestazioni
...
## <a name="python-script-execution-issues"></a>Problemi dell'esecuzione di script Python
...
Questa limitazione verrà risolto in una prossima service release. In alternativa, assicurarsi che la variabile viene inizializzata nello script Python. Qualsiasi valore valido può essere utilizzato, come illustrato negli esempi seguenti:
