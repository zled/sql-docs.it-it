---
title: Installare Machine Learning Server (Standalone) o Microsoft R Server (Standalone) dalla riga di comando | Documenti Microsoft
ms.custom: 
ms.date: 10/31/2017
ms.reviewer: 
ms.suite: sql
...
---
# <a name="install-machine-learning-server-standalone-or-microsoft-r-server-standalone-from-the-command-line"></a>Installare Machine Learning Server (Standalone) o Microsoft R Server (Standalone) dalla riga di comando
...
È necessario sapere eseguire un'installazione da riga di comando di SQL Server e avere familiarità con i relativi argomenti di scripting.

Per altre informazioni, vedere [installazione di SQL Server dal prompt dei comandi](../../database-engine/install-windows/install-sql-server-from-the-command-prompt.md).

Se si installa Machine Learning Server o Microsoft R Server (Standalone) in un computer dotato di senza accesso a Internet, è necessario scaricare i componenti necessari di R (o Python) in anticipo e copiarli in una cartella locale. Per percorsi di download, vedere [installazione dei componenti di machine learning senza accesso a internet](installing-ml-components-without-internet-access.md).


## <a name="bkmk_mls2017"></a> Installare Microsoft di Machine Learning Server (Standalone)
...
+ È possibile installare una lingua specifica, o sia R e Python, ma è necessaria per ogni una licenza separata.
...
+ Se si utilizza l'argomento **funzionalità = SQL_SHARED_MR**, vengono installati solo i componenti Server di Machine Learning, con R né Python. Questa installazione include tutti i prerequisiti, vengono installati automaticamente per impostazione predefinita. Tuttavia, è consigliabile aggiungere almeno una lingua quando si installa per la prima volta.
...
+ **IACCEPTPYTHONLICENSETERMS** indica si hanno accettato le condizioni di licenza per l'utilizzo dei componenti di Python.
...
Per configurare un'altra istanza di Microsoft R Server con gli stessi parametri, è possibile utilizzare nuovamente il file di configurazione che viene creato durante l'installazione. Per altre informazioni, vedere [installazione di SQL Server utilizzando un file di configurazione](../../database-engine/install-windows/install-sql-server-using-a-configuration-file.md).

### <a name="review-installed-components"></a>Componenti revisione installato
...
Per impostazione predefinita, tutti del programma di installazione riepiloghi e i registri per SQL Server e le funzionalità correlate vengono create nelle cartelle seguenti:

+ SQL Server 2017: `C:\Program Files\Microsoft SQL Server\140\Setup Bootstrap\Log`
+ SQL Server 2016:  `C:\Program Files\Microsoft SQL Server\130\Setup Bootstrap\Log`

Viene creata una sottocartella separata per ogni funzionalità che è stato installato.

### <a name="customize-the-r-or-python-environment"></a>Personalizzare l'ambiente R o Python

Dopo l'installazione, è possibile installare pacchetti R o Python aggiuntivi. Il processo è leggermente diversa a seconda se si utilizza SQL Server 2016 o SQL Server 2017.

In SQl Server 2017, è possibile installare e gestire i pacchetti R tramite T-SQL. Per altre informazioni, vedere [installare e gestire i pacchetti R](../r/install-additional-r-packages-on-sql-server.md).
...
### <a name="upgrading-r-server-or-sql-server-machine-learning"></a>L'aggiornamento di apprendimento automatico R Server o SQL Server

Se non si intende utilizzare SQL Server, è possibile installare Microsoft R Server o Server di Machine Learning usando un programma di installazione di Windows separato. Per i percorsi di download e istruzioni, vedere i collegamenti seguenti:
...
Il programma di installazione di windows separato per Machine Learning Server può inoltre essere utilizzato per aggiornare di machine learning i componenti associati all'istanza.  Per ulteriori informazioni, vedere i collegamenti seguenti:
