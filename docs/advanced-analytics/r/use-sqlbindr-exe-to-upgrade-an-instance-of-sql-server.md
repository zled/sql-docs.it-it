...
In questo articolo viene illustrato il processo di *associazione* un'istanza di (In-Database) di SQL Server Machine Learning Services o SQL Server R Services a Microsoft Machine Learning Server allo scopo di aggiornamento dei pacchetti R e Python in un rilasci più veloce. 

Il processo di associazione delle modifiche il meccanismo di aggiornamento del servizio. Senza l'associazione, la versione dei pacchetti R e Python vengono aggiornati solo quando si installa un service pack o aggiornamento cumulativo (CU). Con l'associazione, le versioni più recenti del pacchetto possono essere applicate all'istanza, in modo indipendente dalla pianificazione del rilascio CU.

Associazione interessa solo i componenti di Machine Learning o R dell'istanza del motore di database, non l'istanza di motore di database stesso. Si applica solo a un'istanza di (In-Database). Un'installazione (Standalone) non è incluso nell'ambito.

Se in qualsiasi momento si desidera ripristinare alla manutenzione di SQL Server per l'apprendimento automatico dei componenti, è possibile _disassociare_ l'istanza come descritto in [in questa sezione](#bkmk_Unbind)e disinstallare Server Machine Learning.
