---
title: Sicurezza per SQL Server machine learning e R | Microsoft Docs
ms.prod: sql
...
Oggetto [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] account di accesso o un account utente di Windows è necessario per eseguire gli script R che usano dati di SQL Server o che vengono eseguite con SQL Server come contesto di calcolo. Questo requisito si applica a entrambe [!INCLUDE[rsql_productname_md](../../includes/rsql-productname-md.md)] e SQL Server 2017 [!INCLUDE[rsql-productnamenew-md](../../includes/rsql-productnamenew-md.md)].

Identifica l'account utente o account di accesso di *entità di sicurezza*, che potrebbe essere necessario più livelli di accesso, a seconda dei requisiti di script R:

+ Autorizzazione per accedere al database in cui è abilitato R
...
+ La possibilità di creare nuovi oggetti, ad esempio tabelle, stored procedure che utilizzano lo script R o personalizzato funzioni che il processo di usare R
+ Diritto di installare nuovi pacchetti nel computer di SQL Server, o i pacchetti di usare R forniti a un gruppo di utenti. 

Pertanto, ogni persona che esegue il codice R usando [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] come l'esecuzione del contesto deve essere mappato a un account di accesso nel database. In sicurezza di SQL Server, è in genere più semplice creare i ruoli per gestire i set di autorizzazioni e assegnare gli utenti a tali ruoli, anziché impostare separatamente le autorizzazioni utente. 

Ad esempio, si supponga che è stato creato codice R che viene eseguito sul computer portatile e si desidera eseguire il codice in [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)]. È possibile farlo solo se vengono soddisfatte queste condizioni:
...
Dopo che l'account di accesso o l'account utente di Windows è stato effettuato il provisioning e le autorizzazioni necessarie, è possibile eseguire codice R [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] usando un oggetto origine dati R o chiamando una stored procedure. Ogni volta che R viene avviato da [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)], la sicurezza del motore di database Ottiene il contesto di sicurezza dell'utente che ha avviato il processo R o è stata eseguita la stored procedure e gestisce i mapping dell'utente o account di accesso agli oggetti a protezione diretta. 

Di conseguenza, tutti i processi R avviati da un client remoto devono specificare le informazioni sull'account di accesso o utente come parte della stringa di connessione.
...
Il mapping di un utente esterno di Windows o un account di accesso SQL valido per un account di lavoro è valido solo per la durata della durata della query SQL che esegue lo script R.
