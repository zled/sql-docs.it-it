...
|StockItemHoldings|Colonne non temporali per le voci. Queste sono le colonne aggiornate di frequente.|
...
Le stored procedure in questo schema vengono utilizzate dal processo ETL. Ottenere i dati necessari da diverse tabelle per l'intervallo di tempo necessari per il [pacchetto ETL](wide-world-importers-perform-etl.md).
