...
L'esempio seguente usa `OBJECT_ID` e `sys.indexes` catalogo per ottenere l'ID di tabella e l'ID di indice per il `AK_Product_Name` indice del `Production.Product` tabella il [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] database.
