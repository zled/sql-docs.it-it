---
title: DBCC INDEXDEFRAG (Transact-SQL) | Documenti Microsoft
ms.custom: 
ms.date: 07/16/2017
...
Nell'esempio seguente consente di deframmentare tutte le partizioni del `PK_Product`_`ProductID` indicizzare il `Production.Product` tabella il `AdventureWorks` database.
