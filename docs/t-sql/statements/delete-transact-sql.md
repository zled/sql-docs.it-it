...
#### <a name="c-using-a-cursor-to-determine-the-row-to-delete"></a>C. Utilizzo di un cursore per determinare la riga da eliminare  
 Nell'esempio seguente viene eliminata una riga dal `EmployeePayHistory` tabella il [!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] database utilizzando un cursore denominato `my`_`cursor`. L'operazione di eliminazione interessa unicamente la riga attualmente recuperata dal cursore.  
