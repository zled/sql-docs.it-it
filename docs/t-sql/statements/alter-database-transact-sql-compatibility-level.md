...
> [!NOTE]  
>  **Database SQL di Azure** V12 è stato rilasciato nel dicembre 2014. Un aspetto di tale versione è che i database appena creati hanno il livello di compatibilità impostato su 120. Nel 2015 SQL Database ha iniziato il supporto per il livello 130, anche se il valore predefinito è rimasto 120.  
> 
> A partire dalla **metà di giugno 2016**, nei Database di SQL Azure, il livello di compatibilità predefinito è 130 anziché 120 per i database **appena creati**. I database esistenti creati prima di tale periodo non sono interessati e mantengono il livello di compatibilità corrente (100, 110 e 120). 
> 
...
|Impostazione a livello di compatibilità minore o uguale di 130|Impostazione a livello di compatibilità di 140|  
|--------------------------------------------------|-----------------------------------------|  
...
|Le query in modalità batch che comporteranno spill su disco potrebbero avere problemi in esecuzioni consecutive.|Le query in modalità batch che comporteranno spill su disco potrebbero avere prestazioni migliorate in esecuzioni consecutive. Questa opzione è abilitata tramite **commenti e suggerimenti di concessioni di memoria in modalità batch** che aggiornerà le dimensioni della concessione di memoria di un piano memorizzato nella cache se gli spill si sono verificati per gli operatori in modalità batch. |
|Le query in modalità batch che richiedono una quantità di memoria eccessiva e che comportano problemi di concorrenza potrebbero continuare ad avere problemi in esecuzioni consecutive.|Le query in modalità batch che richiedono una quantità di memoria eccessiva e che comportano problemi di concorrenza in esecuzioni consecutive vedono migliorata la gestione della concorrenza. Questa opzione è abilitata tramite **commenti e suggerimenti di concessioni di memoria in modalità batch** che aggiorna le dimensioni della concessione di memoria di un piano memorizzato nella cache, se originariamente è stata richiesta una quantità eccessiva.|
|Le query in modalità batch contenenti gli operatori di join sono idonee per i tre algoritmi di join fisico, tra cui ciclo nidificato, hash join e join di tipo merge.  Se le stime della cardinalità non sono corrette per gli input di join, è possibile che venga selezionato un algoritmo di join non appropriato.  In questo caso, le prestazioni ne risentiranno e l'algoritmo di join non appropriato rimarrà in uso fino a quando non viene ricompilato il piano memorizzato nella cache.|Si ha un operatore di join aggiuntivo denominato **join adattivo**. Se le stime della cardinalità non sono corrette per l'input di compilazione outer join, è possibile che venga selezionato un algoritmo di join non appropriato.  Se l'istruzione è idoneo per un join adattivo verrà utilizzato un ciclo annidato per gli input di join più piccoli e un hash join per gli input di join più grandi in modo dinamico senza richiedere la ricompilazione. |
...
|Le TVF con istruzioni multiple non dispongono di esecuzione interleave |L'esecuzione interleave è disponibile per TVF con istruzioni multiple al fine di migliorare la qualità del piano. |

Le correzioni derivanti dal flag di traccia 4199 nelle versioni precedenti di SQL Server prima di SQL Server 2017 sono ora abilitate per impostazione predefinita. Con la modalità di compatibilità 140. Il flag di traccia 4199 continuerà a essere applicabile per le nuove correzioni di query optimizer che vengono rilasciate dopo SQL Server 2017. Per informazioni sui Flag di traccia 4199, vedere [Flag di traccia 4199](https://support.microsoft.com/en-us/kb/974006).  
...
|Lo strumento di stima della cardinalità di SQL 2014 è stato introdotto tramite **CardinalityEstimationModelVersion = "120"**|Ulteriori  miglioramenti di stima di cardinalità (CE) con la compatibilità 130, visibile da un piano di query. **CardinalityEstimationModelVersion = "130"**|  
|Modalità batch vs modalità di riga con gli indici Columnstore<br /><br /> Gli ordinamenti in una tabella con indice Columnstore sono in modalità riga<br /><br /> Le aggregazioni su windowed function operano in modalità a riga, come LAG o LEAD<br /><br /> Le query sulle tabelle Columnstore con più clausole distinte operano in modalità riga<br /><br /> Le query in esecuzione MAXDOP 1 o con un piano seriale vengono eseguite in modalità riga | Modalità batch vs modalità di riga con gli indici Columnstore<br /><br /> Gli ordinamenti in una tabella con indice Columnstore sono ora in modalità batch<br /><br /> Le aggregazioni su windowed function operano in modalità batch, come LAG o LEAD<br /><br /> Le query sulle tabelle Columnstore con più clausole distinte operano in modalità batch<br /><br /> Le query in esecuzione MAXDOP 1 o con un piano seriale vengono eseguite in modalità batch|  
...
Le correzioni derivanti dal flag di traccia 4199 nelle versioni precedenti di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] prima di [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] sono ora abilitati per impostazione predefinita. Con la modalità di compatibilità 130. Il flag di traccia 4199 sarà comunque applicabile per nuove correzioni di query optimizer che vengono rilasciate dopo [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]. Per utilizzare il query optimizer precedente in [!INCLUDE[ssSDS](../../includes/sssds-md.md)] è necessario selezionare il livello di compatibilità 110. Per informazioni sui flag di traccia 4199, vedere [Flag di traccia 4199](https://support.microsoft.com/en-us/kb/974006).  
...
|Nei livelli di compatibilità inferiori a 120 l'impostazione della lingua viene ignorata durante la conversione di una **data** in un valore stringa. Si noti che questo comportamento è specifico solo per il tipo **data**. Vedere l'esempio B nella sezione esempi riportata di seguito.|L'impostazione della lingua non viene ignorato durante la conversione di una **data** in un valore stringa.|  
...
|Il comando **SELECT INTO** crea un'operazione di inserimento a thread singolo.|Il comando **SELECT INTO** crea potenzialmente un'operazione di inserimento parallela. Quando si inserisce un numero elevato di righe, con un'operazione parallela è possibile migliorare le prestazioni.|  
...
### <a name="a-changing-the-compatibility-level"></a>A. Modifica del livello di compatibilità  
 L'esempio seguente modifica il livello di compatibilità di [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] a `110,` [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)].  
