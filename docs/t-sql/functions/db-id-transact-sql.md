...
L'esempio seguente usa `DB_ID` per restituire l'ID di database di [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] database nella funzione di sistema `sys.dm_db_index_operational_stats`. La funzione utilizza un ID di database come primo parametro.
