---
title: DB_ID (Transact-SQL) | Documenti Microsoft
ms.custom: 
ms.date: 07/30/2017
...
'*database_name*'  
Nome del database di cui si desidera ottenere l'ID corrispondente. *database_name* è **sysname**. Se *database_name* viene omesso, viene restituito l'ID del database corrente.
...
L'esempio seguente restituisce l'ID di database di [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] database.
...
L'esempio seguente usa `DB`_`ID` per restituire l'ID di database di [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] database nella funzione di sistema `sys.dm_db` \_ `index` \_ `operational` \_ `stats`. La funzione utilizza un ID di database come primo parametro.
...
Nell'esempio seguente viene restituito l'ID di database del database AdventureWorksDW2012.
