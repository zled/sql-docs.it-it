...
ms.topic: "article"
helpviewer_keywords: 
  - "istruzioni [SQL Server], prompt dei comandi"
  - "QUIT - comando"
  - "istruzioni Transact-SQL, prompt dei comandi"
  - "EXIT - comando"
  - "sqlcmd - comandi"
  - "ED - comando"
  - "sqlcmd - utilità"
  - "utilità del prompt dei comandi [SQL Server], sqlcmd"
  - "!! comando"
  - "stored procedure [SQL Server], prompt dei comandi"
  - "stored procedure di sistema [SQL Server], prompt dei comandi"
  - "utilità sqlcmd, informazioni sull'utilità sqlcmd"
  - "script [SQL Server], prompt dei comandi"
  - "RESET - comando"
  - "GO - comando"
...
[!INCLUDE[tsql-appliesto-ss2008-asdb-asdw-pdw-_md](../includes/tsql-appliesto-ss2008-asdb-asdw-pdw-md.md)]
  ...
  L'utilità **sqlcmd** consente di immettere istruzioni Transact-SQL, procedure di sistema e file script al prompt dei comandi, nell'**editor di query** in modalità SQLCMD, in un file script Windows o in un passaggio di processo del sistema operativo (Cmd.exe) in un processo di [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] Agent. e utilizza ODBC per eseguire batch [!INCLUDE[tsql](../includes/tsql-md.md)]. 
  
> [!NOTE]
> Le versioni più recenti dell'utilità sqlcmd sono disponibili come versione Web nell'[Area download](http://go.microsoft.com/fwlink/?LinkID=825643). Per supportare la funzionalità Always Encrypted (`-g`) e l'autenticazione di Azure Active Directory (`-G`) è necessaria almeno la versione 13.1. Nel computer potrebbero essere installate diverse versioni di sqlcmd.exe. Assicurarsi di usare la versione corretta. Per determinare la versione, eseguire `sqlcmd -?`.
...
> [!IMPORTANT] [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] (SSMS) usa [!INCLUDE[msCoName](../includes/msconame-md.md)][!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)]SqlClient per l'esecuzione in modalità normale e SQLCMD nell'**editor di query**. Se **sqlcmd** viene eseguita dalla riga di comando, **sqlcmd** usa il driver ODBC. Poiché possono essere applicate opzioni predefinite diverse, l'esecuzione della stessa query nella modalità SQLCMD di [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] e nell'utilità **sqlcmd** potrebbe generare risultati diversi.  
>   
 ...
 **-E**  
 Utilizza una connessione trusted anziché un nome utente e una password per l'accesso a [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]. In caso di omissione di **-E**, **sqlcmd** usa l'opzione della connessione trusted per impostazione predefinita.  
...
> [!IMPORTANT]L'opzione **-G** si applica solo al database SQL di Azure e ad Azure Data Warehouse. 
 ...
 **-I**  
 Imposta l'opzione di connessione SET QUOTED_IDENTIFIER su ON. Per impostazione predefinita, l'opzione è impostata su OFF. Per altre informazioni, vedere [SET QUOTED_IDENTIFIER &#40;Transact-SQL&#41;](../Topic/SET%20QUOTED_IDENTIFIER%20\(Transact-SQL\).md).  
...
## <a name="remarks"></a>Osservazioni  
 ...
 Per usare **sqlcmd** in modo interattivo, digitare **sqlcmd** al prompt dei comandi con una o più delle opzioni precedentemente descritte in questo argomento. Per altre informazioni, vedere [Utilizzo dell'utilità sqlcmd](../Topic/Use%20the%20sqlcmd%20Utility.md)  
...
-   In **:Error**, **:Out** e **:Perftrace** è consigliabile usare valori **\<***filename***>** distinti. Se viene usato lo stesso valore **\<***filename***>**, è possibile che gli input di tali comandi vengano confusi.  
...
###  <a name="a-nameoutputxmla-xml-output-format"></a><a name="OutputXML"></a> Formato di output XML  
...
###  <a name="a-nameoutputjsona-json-output-format"></a><a name="OutputJSON"></a> Formato di output JSON  
...
## <a name="see-also"></a>Vedere anche  
 [Avvio dell'utilità sqlcmd](../Topic/Start%20the%20sqlcmd%20Utility.md)   
 [Esecuzione di file script Transact-SQL mediante sqlcmd](../Topic/Run%20Transact-SQL%20Script%20Files%20Using%20sqlcmd.md)   
 [Utilizzo dell'utilità sqlcmd](../Topic/Use%20the%20sqlcmd%20Utility.md)   
 [Utilizzo di sqlcmd con variabili di scripting](../Topic/Use%20sqlcmd%20with%20Scripting%20Variables.md)   
 [Connessione al Motore di database tramite sqlcmd](../Topic/Connect%20to%20the%20Database%20Engine%20With%20sqlcmd.md)   
 [Modifica di script SQLCMD con l'editor di query](../Topic/Edit%20SQLCMD%20Scripts%20with%20Query%20Editor.md)   
 [Gestire passaggi di processo](../Topic/Manage%20Job%20Steps.md)   
 [Creare un passaggio di processo CmdExec](../Topic/Create%20a%20CmdExec%20Job%20Step.md)  
