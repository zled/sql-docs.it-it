...
ms.topic: "article"
helpviewer_keywords: 
  - "Utilità SqlLocalDB [SQL Server]"
  - "utilità di runtime del database locale"
  - "LocalDB, utilità SqlLocalDB"
 ...
 *\<instance-name>* restituisce il nome, la versione, lo stato (In esecuzione o Arrestato), l'ultima ora di inizio per l'istanza specificata di **LocalDB** di [!INCLUDE[ssExpress](../includes/ssexpress-md.md)] e il nome della pipe locale di **LocalDB**.  
...
### A. Creazione di un'istanza di LocalDB.  
...
[Strumento di gestione della riga di comando: SqlLocalDB.exe](Command-Line%20Management%20Tool:%20SqlLocalDB.exe.md)  
