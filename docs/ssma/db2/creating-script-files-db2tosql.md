...
7.  **Operazione di arresto:** durante l'operazione a metà, se l'utente desidera arrestare l'operazione, quindi **'Ctrl + C'** tasti di scelta rapida può essere utilizzato. SSMA per DB2 Console attenderà il completamento dell'operazione e termina l'esecuzione della console.  
...
Ad esempio, una migrazione tipica dei dati di una tabella specifica in un database DB2 segue la gerarchia dei: Schema -&gt; tabella.  
