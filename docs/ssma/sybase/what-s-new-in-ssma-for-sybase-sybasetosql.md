...
- SSMA per SAP ASE è stata migliorata con le correzioni di destinazione che consentono di migliorare le metriche di qualità e la conversione.
- In base a grande richiesta, la versione a 32 bit di SSMA per SAP ASE è nuovo. Se confrontato con l'implementazione precedente (prima v7.4), sono disponibili due pacchetti di installazione, ma non possono essere installati side-by-side. Di conseguenza, è necessario scegliere la versione più appropriata in base ai componenti di connettività che è. È sempre preferibile usare la versione a 64 bit, se possibile.
...
La versione v7.5 di SSMA per SAP ASE contiene le seguenti modifiche:
-   Migliorato con numerosi miglioramenti per garantire maggiore accessibilità per persone affette da disabilità.
-   Aggiornato per fornire il supporto per la sintassi di creare o sostituire.
