---
title: "Miglioramento delle prestazioni di indici full-text | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "prestazioni [SQL Server], ricerca full-text"
  - "query full-text [SQL Server], prestazioni"
  - "ricerche per indicizzazione [ricerca full-text]"
  - "indici full-text [SQL Server], prestazioni"
  - "ricerca full-text [SQL Server], prestazioni"
  - "batch [SQL Server], ricerca full-text"
...
##  <a name="causes"></a> Cause comuni di problemi di prestazioni  
...
-   In caso di memoria fisica insufficiente, il collo di bottiglia è rappresentato dalla memoria.  
...
-   Per usare al meglio tutti i processori o i core, impostare [sp_configure](../../relational-databases/system-stored-procedures/sp-configure-transact-sql.md)'**max full-text crawl ranges**' sul numero di CPU nel sistema. Per informazioni su questa opzione di configurazione, vedere [Opzione di configurazione del server max full-text crawl range](../../database-engine/configure-windows/max-full-text-crawl-range-server-configuration-option.md).  
...
##  <a name="full"></a> Risoluzione dei problemi relativi alle prestazioni di popolamenti completi  
 Per diagnosticare problemi di prestazioni, analizzare i log della ricerca per indicizzazione full-text. Per informazioni sui log di ricerca per indicizzazione, vedere [Popolamento degli indici full-text](../../relational-databases/search/populate-full-text-indexes.md).  
...
> [!NOTE]  
>  Per informazioni sui buffer di memoria usati da una ricerca per indicizzazione full-text, vedere [sys.dm_fts_memory_buffers &#40;Transact-SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-fts-memory-buffers-transact-sql.md).  
...
|x64|*F* **=** *numero di intervalli di ricerca per indicizzazione* **\*** 10 **\*** 8|*M* **=** *T* **–** *F* **–** 500|  
 ...
 (3) Si presuppone che *ism_size* sia 8 MB per le piattaforme x64.  
...
##  <a name="filters"></a> Risoluzione dei problemi relativo alla riduzione delle prestazioni di indicizzazione a causa di filtri  
