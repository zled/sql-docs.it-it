...
ms.topic: "article"
helpviewer_keywords: 
  - "ripristini di file [SQL Server]"
  - "modello di recupero con registrazione minima [SQL Server]"
  - "file di ripristino [SQL Server], sequenza di ripristino di Transact-SQL"
  - "ripristino di file [SQL Server]"
  - "sequenza di ripristino di Transact-SQL"
  - "ripristino dei file [SQL Server], modello di recupero con registrazione minima"
  - "ripristini di file [SQL Server], modello di recupero con registrazione minima"
  - "ripristini di file [SQL Server], sequenza di ripristino di Transact-SQL"
...
-   Ripristino di file online  
     ...
     Per informazioni sul supporto per il ripristino di pagine e file online, vedere [Funzionalità supportate dalle edizioni di SQL Server 2016](../Topic/Features%20Supported%20by%20the%20Editions%20of%20SQL%20Server%202016.md). Per altre informazioni sui ripristini online, vedere [Ripristino in linea &#40;SQL Server&#41;](../../relational-databases/backup-restore/online-restore-sql-server.md).  
  
    > [!TIP]  
    >  Se si vuole attivare la modalità offline per il database al fine di eseguire un ripristino di file, attivare la modalità offline per il database prima di avviare la sequenza di ripristino eseguendo l'istruzione [ALTER DATABASE](../Topic/ALTER%20DATABASE%20SET%20Options%20\(Transact-SQL\).md) seguente: ALTER DATABASE *nome_database* SET OFFLINE.  
...
### Passaggi di Transact-SQL per la sequenza di ripristino di file (modello di recupero con registrazione minima)  
 Questa sezione descrive le opzioni [RESTORE](../Topic/RESTORE%20\(Transact-SQL\).md) essenziali di [!INCLUDE[tsql](../../includes/tsql-md.md)] per una semplice sequenza di ripristino di file. La sintassi e i dettagli non rilevanti sono stati omessi.  
...
-   [Metodo Restore.SqlRestore (server)](../Topic/SqlRestore%20Method.md) (SMO)  
