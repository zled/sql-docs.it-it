...
-   Se si ha un account di Azure,  fare clic **[qui](https://azure.microsoft.com/marketplace/partners/microsoft/sqlserver2016ctp3evaluationwindowsserver2012r2/?wt.mc_id=sqL16_vm)** per creare rapidamente una macchina virtuale in cui è già installato [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
  
## Uso degli snapshot di Azure per eseguire il backup dei file di database archiviati in Azure  
  
### Che cos'è un backup di snapshot di file [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]  
...
## Ripristino da un backup di snapshot di file [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]  
...
## Ripristino temporizzato da un backup di snapshot di file [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]  
...
## Eliminazione di un set di backup di snapshot di file di database  
 Per eliminare un set di backup di snapshot di file, usare la stored procedure di sistema **sys.sp_delete_backup**. Specificare il nome del database in modo che il sistema verifichi che il set di backup di snapshot di file specificato sia effettivamente un backup per il database specificato. Se non si specifica alcun nome di database, il set di backup specificato con i relativi snapshot di file verrà eliminato senza tale convalida. Per altre informazioni, vedere [sp_delete_backup &#40;Transact-SQL&#41;](../Topic/sp_delete_backup%20\(Transact-SQL\).md).  
...
## Visualizzazione di snapshot di file di backup del database  
