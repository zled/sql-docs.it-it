...
|bounding_box_xmin|**float(53)**|Coordinata X dell'angolo inferiore sinistro del riquadro casella, uno di: NULL = non applicabile per uno schema a mosaico specifico (ad esempio GEOGRAPHY_GRID)  *n*  = se tessellation_scheme è GEOMETRY_GRID, la coordinata x-min valore.                     **Nota:** le coordinate definite dai parametri del rettangolo di interpretazione per ogni oggetto in base al relativo [identificatore SRID (Spatial Reference)](../../relational-databases/spatial/spatial-reference-identifiers-srids.md).|  
|bounding_box_ymin|**float(53)**|Coordinata Y dell'angolo inferiore sinistro del riquadro casella, uno di: NULL = non applicabile per uno schema a mosaico specifico (ad esempio GEOGRAPHY_GRID)  *n*  = se tessellation_scheme è GEOMETRY_GRID, la coordinata y-min valore|  
|bounding_box_xmax|**float(53)**|Coordinata X dell'angolo superiore sinistro del riquadro casella, uno di: NULL = non applicabile per uno schema a mosaico specifico (ad esempio GEOGRAPHY_GRID)  *n*  = se tessellation_scheme è GEOMETRY_GRID, la coordinata x-max valore|  
