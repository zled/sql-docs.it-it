...
description: "Panoramica del supporto BYOK per Transparent Data Encryption con Azure Key Vault per i database e data warehouse SQL. Il documento descrive i vantaggi offerti dalla funzionalità, come funziona oltre a offrire considerazioni e consigli."
keywords: 
...
- Separazione della gestione delle chiavi da quella dei dati all'interno dell'organizzazione, per supportare la separazione dei ruoli
...
- **Rotazioni delle chiavi:** le protezioni TDE devono essere ruotate in base alle disposizioni interne o ai requisiti di conformità. Le rotazioni delle chiavi possono essere eseguite usando l'insieme di credenziali delle chiavi della protezione TDE.  
...
- **Eliminazione di chiavi**: le chiavi possono essere eliminate da Key Vault e dal server SQL per ottimizzare la protezione o rispettare la conformità.
...
Dal momento in cui un server non ha più accesso alla protezione TDE, perché sono state rimosse autorizzazioni di Key Vault o una chiave è stata eliminata, **tutte le connessioni ai database crittografati nel server vengono bloccate e i database vengono portati offline ed eliminati entro 24 ore**. I backup meno recenti crittografati con la chiave non disponibile non sono più accessibili. Nel caso estremo in cui si sospetta che una chiave sia compromessa, ad esempio se un servizio o un utente accede alla chiave senza autorizzazione, è consigliabile eliminare la chiave seguendo le istruzioni su come [rimuovere una chiave potenzialmente compromessa](transparent-data-encryption-byok-azure-sql-remove-tde-protector.md). I database devono essere eliminati prima di eliminare una protezione TDE attiva per evitare potenziali perdite di dati relativi a fino 10 minuti di attività.  

### <a name="expired-keys"></a>Chiavi scadute

Poiché la disponibilità della protezione TDE influisce direttamente sulla disponibilità del database, non è consentito aggiungere una chiave con data di scadenza a un server SQL. Se una chiave viene già usata come protezione TDE per un server e successivamente viene aggiunta una data di scadenza in Azure Key Vault, **dopo la scadenza della chiave i database crittografati non avranno più accesso alla relativa protezione TDE e verranno eliminati entro 24 ore.**
...
Le protezioni TDE univoche per un database o data warehouse non sono supportate. La protezione TDE è impostata a livello di server ed ereditata da tutte le risorse di quel server. 
...
Esistono due modalità per configurare la replica geografica per i server con Key Vault: 
