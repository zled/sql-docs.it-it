...
## <a name="arguments"></a>Argomenti  
 [  **@publisher=** ] **'***publisher***'**  
 Nome del server di pubblicazione da eliminare. *server di pubblicazione* è **sysname**, non prevede alcun valore predefinito.  
  
 [  **@no_checks=** ] *no_checks*  
 Specifica se **sp_dropdistpublisher** verifica che il server di pubblicazione sia stato disinstallato il server come server di distribuzione. *no_checks* è **bit**, il valore predefinito è **0**.  
 ...
 [  **@ignore_distributor=** ] *ignore_distributor*  
 Specifica se gli oggetti di distribuzione vengono mantenuti nel server di distribuzione quando il server di pubblicazione viene rimosso. *ignore_distributor* è **bit** e può essere uno dei valori seguenti:  
