...
## <a name="arguments"></a>Argomenti  
 ...
 [ @publication_id **=**] **'***publication_id***'** OUTPUT  
 Numero di identificazione della pubblicazione. *publication_id* viene **uniqueidentifier** e un parametro di OUTPUT, con un valore predefinito è NULL.  
  
 [ @reserved **=**] **'***riservato***'**  
 [!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)] *riservata* viene **nvarchar(20)**, con un valore predefinito è NULL.  
...
|sync_mode|**tinyint**|Modalità di sincronizzazione della pubblicazione:<br /><br /> **0** = programma per la copia bulk nativa (**bcp** utilità)<br /><br /> **1** = copia bulk di carattere|  
...
|allow_anonymous|**int**|Determina se è possibile creare sottoscrizioni anonime per la pubblicazione specificata. **0** indica che una sottoscrizione anonima non è consentita.|  
...
|snapshot_jobid|**binary(16)**|ID di processo dell'agente snapshot. Per ottenere la voce per il processo di snapshot nella [sysjobs](../../relational-databases/system-tables/dbo-sysjobs-transact-sql.md) tabella di sistema, è necessario convertire il valore esadecimale a **uniqueidentifier**.|  
|enabled_for_internet|**int**|Viene determinato se la pubblicazione è abilitata per Internet. Se **1**, i file di sincronizzazione per la pubblicazione vengono inseriti i `C:\Program Files\Microsoft SQL Server\MSSQL\Repldata\Ftp` directory. La directory FTP (File Transfer Protocol) deve essere creata dall'utente. Se **0**, la pubblicazione non è abilitata per l'accesso a Internet.|  
