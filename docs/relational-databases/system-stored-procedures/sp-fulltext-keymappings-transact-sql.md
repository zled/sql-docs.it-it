---
title: sp_fulltext_keymappings (Transact-SQL) | Documenti Microsoft
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
|Key|*|Valore di chiave full-text dalla tabella specificata.<br /><br /> Se nella tabella di mapping non esiste alcuna chiave full-text, viene restituito un set di righe vuoto.|  
 ...
 <sup>*</sup> Il tipo di dati per la chiave è identico al tipo di dati della colonna chiave full-text nella tabella di base.  
...
|*table_id*, NULL, *chiave*|Se sono presenti tre parametri, il secondo parametro deve essere NULL, e *chiave* deve essere diverso da null e specificare un valore di chiave full-text valido dalla tabella specificata. Questa funzione è utile per isolare il valore DocId corrispondente a una chiave full-text specifica della tabella di base.|  
...
> [!NOTE]  
>  Gli esempi in questa sezione utilizzano il `Production.ProductReview` sommario il [!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] database di esempio. È possibile creare questo indice eseguendo l'esempio fornito per il `ProductReview` nella tabella [CREATE FULLTEXT INDEX &#40;Transact-SQL&#41;](../../t-sql/statements/create-fulltext-index-transact-sql.md).  
