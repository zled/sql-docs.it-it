---
title: sp_statistics (Transact-SQL) | Documenti di Microsoft
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
## <a name="arguments"></a>Argomenti  
 ...
 [  **@index_name=** ] **'***index_name***'**  
 Nome dell'indice. *index_name* è **sysname**, con valore predefinito è %. La ricerca con caratteri jolly è supportata.  
 ...
 Il valore **E** (SQL_ENSURE) richiede il driver per recuperare in modo incondizionato le statistiche.  
  
 Il valore **Q** (SQL_QUICK) richiede il driver per recuperare la cardinalità e pagine solo se sono immediatamente disponibili dal server. In tal caso, il driver non garantisce che i valori siano aggiornati. Per le applicazioni scritte in base allo standard Open Group viene restituito sempre il comportamento di SQL_QUICK da driver conformi a ODBC 3.x.  
...
|**INDEX_QUALIFIER**|**sysname**|Nome del proprietario dell'indice. In alcuni prodotti DBMS gli indici possono essere creati da utenti diversi dal proprietario della tabella. In [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], questa colonna è sempre lo stesso **TABLE_NAME**.|  
...
|**COLUMN_NAME**|**sysname**|Per ogni colonna del nome della colonna di **TABLE_NAME** restituito. In questa colonna viene sempre restituito un valore.|  
 ...
 **sp_statistics** è equivalente a **SQLStatistics** in ODBC. I risultati restituiti vengono ordinati in **NON_UNIQUE**, **tipo**, **INDEX_QUALIFIER**, **INDEX_NAME**, e **SEQ_IN_ INDICE**. Per altre informazioni, vedere la [riferimento all'API ODBC](http://go.microsoft.com/fwlink/?LinkId=68323).  
