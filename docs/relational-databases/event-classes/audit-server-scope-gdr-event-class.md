...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)] La classe di evento **Audit Server Scope GDR** viene generata quando si esegue un'istruzione GRANT, REVOKE o DENY per le autorizzazioni nell'ambito del server, ad esempio la creazione di un account di accesso.  
