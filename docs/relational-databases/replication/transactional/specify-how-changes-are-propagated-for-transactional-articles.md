---
title: "Impostazione della modalit&#224; di propagazione delle modifiche per gli articoli transazionali | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "replica transazionale, metodi di propagazione"
...
-   Specificare che l'azione non deve essere eseguita in alcun Sottoscrittore. In tal caso le transazioni non vengono replicate.  
 ...
 Per impostare il metodo di propagazione per le modifiche di dati negli articoli transazionali, vedere [impostare il metodo di propagazione per le modifiche dei dati negli articoli transazionali](../../../relational-databases/replication/publish/set-the-propagation-method-for-data-changes-to-transactional-articles.md).  
...
-   **sp_Msins _ \<** *tablename* **>**, che gestisce gli inserimenti.  
  
-   **sp_Msupd _ \<** *tablename* **>**, che gestisce gli aggiornamenti.  
  
-   **sp_Msdel _ \<** *tablename* **>**, gestione delle eliminazioni.  
  
 Il **\<***tablename***>** utilizzato nella procedura dipende dalla modalità in cui è stato aggiunto l'articolo alla pubblicazione e se il database di sottoscrizione contiene una tabella con lo stesso nome con un proprietario diverso.  
...
-   Lo schema nel Sottoscrittore è in genere identico allo schema nel server di pubblicazione, ma può essere anche un subset dello schema del server di pubblicazione se viene utilizzato il filtro delle colonne. Tuttavia, se è necessario trasformare lo schema mentre i dati vengono spostati in modo che lo schema nel Sottoscrittore non è un subset dello schema nel server di pubblicazione, [!INCLUDE[ssISCurrent](../../../includes/ssiscurrent-md.md)] è la soluzione consigliata. Per ulteriori informazioni, vedere [SQL Server Integration Services](../../../integration-services/sql-server-integration-services.md).  
  
-   Se si apportano modifiche allo schema in una tabella pubblicata, è necessario rigenerare le procedure personalizzate. Per ulteriori informazioni, vedere [procedure transazionali personalizzate di rigenerare per riflettere le modifiche dello Schema](../../../relational-databases/replication/transactional/regenerate-custom-transactional-procedures-to-reflect-schema-changes.md).  
  
-   Se si utilizza un valore maggiore di 1 per **- SubscriptionStreams** parametro dell'agente di distribuzione, è necessario assicurarsi che vengano completati gli aggiornamenti alle colonne chiave primaria. Esempio:  
...
#### Sintassi SCALL  
 Stored procedure UPDATE  
 Stored procedure che gestiscono istruzioni UPDATE verranno passate i valori aggiornati solo per le colonne modificate, seguiti dai valori originali per le colonne chiave primaria, seguita da una maschera di bit (**Binary (n)**) parametro che indica le colonne modificate. Nell'esempio seguente la colonna 2 (c2) non è stata modificata:  
...
#### Sintassi MCALL  
 Stored procedure UPDATE  
 Stored procedure che gestiscono istruzioni UPDATE verranno passate i valori aggiornati per tutte le colonne definite nell'articolo, seguiti dai valori originali per le colonne chiave primaria, seguite da una maschera di bit (**Binary (n)**) parametro che indica le colonne modificate:  
...
> [!NOTE]  
>  Quando si utilizza la sintassi XCALL, la prima dei valori di immagine per **testo** e **immagine** colonne devono essere NULL.  
