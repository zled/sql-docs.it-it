...
## Rilevamento dei conflitti  
 Il modo in cui i conflitti vengono rilevati per i record logici è determinato da due proprietà di articolo: **column_tracking** e **logical_record_level_conflict_detection**. [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] e versioni successive supportano anche rilevamento a livello di record logico.  
  
 Il **logical_record_level_conflict_detection** articolo può essere impostata su TRUE o FALSE. Il valore deve essere impostato solo per l'articolo padre di livello principale e verrà ignorato per gli articoli figlio. Se questo valore è FALSE, la replica di tipo merge rileva i conflitti delle versioni precedenti di [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)], esclusivamente in base al valore di **column_tracking** proprietà per l'articolo. Se questo valore è TRUE, la replica di tipo merge ignorerà il **column_tracking** proprietà dell'articolo e per rilevare un conflitto se vengono apportate modifiche in qualsiasi punto del record logico. Ad esempio, si consideri lo scenario seguente:  
...
-   **column_tracking** è TRUE o FALSE  
  
-   **logical_record_level_conflict_detection** è FALSE  
  
-   **logical_record_level_conflict_resolution** è FALSE  
...
-   **column_tracking** è TRUE  
  
-   **logical_record_level_conflict_detection** è FALSE  
  
-   **logical_record_level_conflict_resolution** è TRUE  
...
-   **logical_record_level_conflict_detection** è TRUE  
  
-   **logical_record_level_conflict_resolution** è TRUE  
  
 Un server di pubblicazione e un Sottoscrittore dispongono dello stesso set di dati iniziale. Il server di pubblicazione modifica la colonna **custcol1** della tabella **customers** . Il Sottoscrittore modifica la colonna **ordercol1** della tabella **orders** . Sono state apportate modifiche alla stessa riga o le colonne, ma dato che le modifiche vengono eseguite nello stesso record logico per **custid**= 1, le modifiche vengono rilevate come conflitto a livello di record logici.  
