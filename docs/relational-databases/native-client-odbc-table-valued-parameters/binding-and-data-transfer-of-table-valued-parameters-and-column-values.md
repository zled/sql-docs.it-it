...
[!INCLUDE[SNAC_Deprecated](../../includes/snac-deprecated.md)]
 ...
 Nota: per i driver di Linux e Mac ODBC con unixODBC 2.3.1 a 2.3.4, quando si imposta il nome TVP tramite SQLSetDescField con il campo di descrizione SQL_CA_SS_TYPE_NAME, unixODBC non automaticamente la conversione tra ANSI e Unicode le stringhe in base l'esatto funzione chiamata (SQLSetDescFieldA / SQLSetDescFieldW). È necessario sempre utilizzare SQLBindParameter o SQLSetDescFieldW con una stringa Unicode (UTF-16) per impostare il nome TVP.
  
 La ricezione e l'invio di dati effettivi non riguardano il parametro con valori di tabella stesso ma ognuna delle colonne che lo costituiscono. Poiché il parametro con valori di tabella è una pseudo colonna, vengono utilizzati i parametri per la funzione SQLBindParameter per fare riferimento ad attributi diversi dagli altri tipi di dati, come indicato di seguito:  
...
|*ColumnSize*|SQL_DESC_LENGTH o SQL_DESC_PRECISION in IPD.<br /><br /> Questo dipende dal valore di *ParameterType*.|SQL_DESC_ARRAY_SIZE<br /><br /> Può essere impostato anche utilizzando SQL_ATTR_PARAM_SET_SIZE quando lo stato attivo del parametro è impostato sul parametro con valori di tabella.<br /><br /> Per un parametro con valori di tabella, è il numero di righe nei buffer delle colonne dei parametri con valori di tabella.|  
|*DecimalDigits*|SQL_DESC_PRECISION o SQL_DESC_SCALE in IPD.|Non utilizzato. Deve essere 0.<br /><br /> Se questo parametro non verrà SQLBindParameter 0, verrà generato restituito SQL_ERROR e un record di diagnostica con SQLSTATE = HY104 e il messaggio "non valida e precisione o scala".|  
|*ParameterValuePtr*|SQL_DESC_DATA_PTR in APD.|SQL_CA_SS_TYPE_NAME.<br /><br /> Questo parametro è facoltativo per le chiamate di stored procedure ed è possibile specificare NULL se non è richiesto. Deve essere specificato per le istruzioni SQL che non sono chiamate di stored procedure.<br /><br /> Questo parametro serve anche come valore univoco utilizzabile dall'applicazione per identificare il parametro con valori di tabella quando viene utilizzata l'associazione variabile di righe. Per ulteriori informazioni, vedere la sezione "Associazione variabile di righe di parametri con valori di tabella" più avanti n questo argomento.<br /><br /> Quando in una chiamata a SQLBindParameter viene specificato un nome di tipo di parametro con valori di tabella, è necessario specificarlo come valore Unicode, anche in applicazioni che vengono compilate come applicazioni ANSI. Il valore utilizzato per il parametro *StrLen_or_IndPtr* deve essere SQL_NTS o la lunghezza della stringa del nome moltiplicata per sizeof (WCHAR).|  
...
2.  Chiama la funzione SQLSetStmtAttr per impostare SQL_SOPT_SS_PARAM_FOCUS sul numero ordinale di ogni parametro con valori di tabella.  
    ...
    1.  Per ogni parametro con valori di tabella, associa le colonne di parametri con valori di tabella mediante chiamate SQLBindParameter, SQLSetDescRec o SQLSetDescField.  
