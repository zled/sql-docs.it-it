---
title: "Gestione dei metadati quando si rende disponibile un database in un&#39;altra istanza del server (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "query tra database [SQL Server]"
  - "accessi [SQL Server], ricreazione in un'altra istanza del server"
  - "trigger [SQL Server], DLL"
  - "messaggi di errore definiti dall'utente [SQL Server]"
  - "autorizzazioni [SQL Server], accesso ai metadati"
  - "motore di ricerca full-text [SQL Server]"
  - "metadati [SQL Server], database disponibili ad altre istanze"
  - "processi [SQL Server Agent], ricreazione in un'altra istanza del server"
  ...
  - "ripristino [SQL Server], in un'altra istanza del server"
  - "server collegati [SQL Server], metadati"
  - "Provider WMI per gli eventi server, metadati"
  - "collegamento di database [SQL Server]"
  - "log shipping [SQL Server], metadati"
  - "crittografia [SQL Server], metadati"
  - "configurazione server [SQL Server]"
  - "query distribuite [SQL Server], metadati"
  - "stored procedure estese [SQL Server], metadati"
  - "credenziali [SQL Server], metadati"
  - "copia di database"
...
##  <a name="database_ownership"></a> Proprietà dei database  
...
##  <a name="user_defined_error_messages"></a> Messaggi di errore definiti dall'utente  
 I messaggi di errore definiti dall'utente sono contenuti nella vista del catalogo [sys.messages](../Topic/sys.messages%20\(Transact-SQL\).md). Questa vista del catalogo è archiviata nel database **master**. Se un'applicazione del database dipende da messaggi di errore definiti dall'utente e il database è reso disponibile in un'altra istanza del server, usare [sp_addmessage](../../relational-databases/system-stored-procedures/sp-addmessage-transact-sql.md)per aggiungere tali messaggi definiti dall'utente nell'istanza del server di destinazione.  

  
##  <a name="event_notif_and_wmi_events"></a> Notifiche degli eventi ed eventi di Strumentazione gestione Windows (WMI) (a livello del server)  
...
> **IMPORTANTE** [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] Usare invece la funzionalità [Integrazione CLR](../Topic/Common%20Language%20Runtime%20\(CLR\)%20Integration%20Overview.md).  
...
##  <a name="sb_applications"></a> Applicazioni di Service Broker  
...
##  <a name="triggers"></a> Trigger (a livello del server)  
