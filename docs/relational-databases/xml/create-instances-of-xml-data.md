...
ms.topic: "article"
helpviewer_keywords: 
  - "cast dei tipi delle istanze di dati di tipo stringa [XML in SQL Server]"
  - "XML [SQL Server], tipizzato"
  - "tipo di dati XML [SQL Server], generazione di istanze"
  - "cast di istanze di stringa [XML in SQL Server]"
  - "XML tipizzato"
  - "generazione di istanze XML [SQL Server]"
  - "XML [SQL Server], generazione di istanze"
  - "spazio vuoto [XML in SQL Server]"
...
### Cast di tipo di tipi CLR definiti dall'utente  
 Se un tipo CLR definito dall'utente include una serializzazione XML, per le istanze di tale tipo è possibile eseguire il cast in modo esplicito su un tipo di dati XML. Per altri dettagli sulla serializzazione XML di un tipo CLR definito dall'utente, vedere [Serializzazione XML da oggetti di database CLR](../Topic/XML%20Serialization%20from%20CLR%20Database%20Objects.md).  
