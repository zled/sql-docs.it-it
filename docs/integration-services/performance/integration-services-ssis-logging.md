...
helpviewer_keywords: 
  ...
  - "Registro eventi di Windows - provider di log [Integration Services]"
  - "File XML - provider di log [Integration Services]"
  - "SQL Server Profiler, provider di log"
  - "pacchetti di SQL Server Integration Services, log"
  - "pacchetti SSIS, log"
  - "attività [Integration Services], log"
  - "log [Integration Services], provider di log"
  - "pacchetti di Integration Services"
  - "provider di log [Integration Services]"
  - "pacchetti [Integration Services], log"
  - "provider di log per file di testo"
  - "provider di log di SQL Server"
...
# Registrazione di Integration Services (SSIS)
 ...
 È possibile configurare l'ambito di registrazione che si verifica durante l'esecuzione di un pacchetto nel server [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] . Per altre informazioni, vedere [Enable Logging for Package Execution on the SSIS Server](../../integration-services/performance/enable-logging-for-package-execution-on-the-ssis-server.md).  
...
-   [Finestra di dialogo Configura log SSIS](../../integration-services/performance/configure-ssis-logs-dialog-box.md)  
...
-   [Abilitare la registrazione per l'esecuzione di pacchetti nel server SSIS](../../integration-services/performance/enable-logging-for-package-execution-on-the-ssis-server.md)  
