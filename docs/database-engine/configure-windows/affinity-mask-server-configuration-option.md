...
>  Il supporto dell'affinità nei server dotati di un numero di processori compreso tra 33 e 64 è disponibile solo su sistemi operativi a 64 bit.  
 ...
 Per le modifiche alle maschere di bit di affinità, è necessario che [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] abiliti un nuovo scheduler della CPU e disabiliti quello esistente. È quindi possibile elaborare nuovi batch nei nuovi scheduler o in quelli rimanenti.  
  
 Per avviare un nuovo scheduler della CPU, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] crea un nuovo scheduler e lo aggiunge all'elenco di quelli standard. Il nuovo scheduler viene preso in considerazione unicamente per i nuovi batch in entrata. L'esecuzione dei batch correnti continua nello stesso scheduler. I thread di lavoro migrano al nuovo scheduler man mano che si liberano o che ne vengono creati di nuovi.  
  
 Per chiudere uno scheduler, è necessario che tutti i batch gestiti da esso abbiano completato le relative attività e interrompano l'esecuzione. Uno scheduler che è stato chiuso viene contrassegnato come offline, in modo che non venga pianificato alcun nuovo batch su di esso.  
  
 Se viene aggiunto o rimosso un nuovo scheduler, le attività di sistema permanenti quali lockmonitor, checkpoint, i thread delle attività di sistema (elaborazione DTC) e signal process proseguono l'esecuzione mentre il server è operativo. Queste attività di sistema permanenti non eseguono la migrazione in modo dinamico. Per ridistribuire il carico del processore di queste attività sugli scheduler, è necessario riavviare l'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Se [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] tenta di chiudere uno scheduler associato a un'attività di sistema permanente, l'esecuzione dell'attività continua nello scheduler offline (nessuna migrazione). Lo scheduler è associato ai processori nella maschera di affinità modificata e non dovrebbe applicare alcun carico al processore con il quale è stata creata l'affinità prima della modifica. La presenza di scheduler offline aggiuntivi non dovrebbe influire in modo significativo sul carico del sistema. In caso contrario, per riconfigurare queste attività è necessario riavviare il server di database.  
