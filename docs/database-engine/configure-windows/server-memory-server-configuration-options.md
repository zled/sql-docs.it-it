...
helpviewer_keywords: 
  ...
  - "max server memory - opzione"
  - "memoria virtuale [SQL Server]"
  - "VMM"
  - "opzioni di memoria server [SQL Server]"
  - "server [SQL Server], memoria"
  - "pool di buffer [SQL Server]"
  - "min server memory - opzione"
  - "opzioni manuali di memoria [SQL Server]"
  - "memoria [SQL Server], server"
...
## Blocco di pagine in memoria  
 Questi criteri di Windows determinano gli account autorizzati a usare un processo per mantenere i dati nella memoria fisica, impedendo al sistema di eseguire il paging dei dati nella memoria virtuale su disco. Il blocco delle pagine in memoria può garantire il corretto funzionamento del server quando si verifica il paging della memoria su disco. L'opzione di SQL Server **Blocco di pagine in memoria** viene impostata su ON per istanze di [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] Standard edition ed edizioni superiori quando all'account con i privilegi per l'esecuzione di sqlservr.exe è stato concesso il diritto utente di Windows Blocco di pagine in memoria (LPIM).  
...
## Assegnazione della quantità massima di memoria a SQL Server  
 La memoria può essere configurata fino al limite dello spazio degli indirizzi virtuali di processo in tutte le edizioni di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] (8 TB).  
