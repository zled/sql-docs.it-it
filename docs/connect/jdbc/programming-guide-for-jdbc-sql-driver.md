---
title: Guida di programmazione per il Driver SQL JDBC | Documenti Microsoft
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.reviewer: 
ms.suite: 
ms.technology:
- drivers
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 99f0db69-ce1a-49cf-9c24-0b9abfa4e0c3
caps.latest.revision: 2
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.workload: On Demand
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: b30ab2fe38fce6bb53b171d36a1009dea4b44e9f
ms.contentlocale: it-it
ms.lasthandoff: 09/09/2017

---
# <a name="programming-guide-for-jdbc-sql-driver"></a>Guida di programmazione per il Driver SQL JDBC
[!INCLUDE[Driver_JDBC_Download](../../includes/driver_jdbc_download.md)]

* [Connessione a SQL Server con il Driver JDBC](../../connect/jdbc/connecting-to-sql-server-with-the-jdbc-driver.md)
* [Informazioni sui tipi di dati del Driver JDBC](../../connect/jdbc/understanding-the-jdbc-driver-data-types.md)
* [Utilizzo delle istruzioni con il Driver JDBC](../../connect/jdbc/using-statements-with-the-jdbc-driver.md)
* [Gestione dei risultati set con il Driver JDBC](../../connect/jdbc/managing-result-sets-with-the-jdbc-driver.md)
* [Esecuzione di transazioni con il Driver JDBC](../../connect/jdbc/performing-transactions-with-the-jdbc-driver.md)
* [Gestione dei metadati con il Driver JDBC](../../connect/jdbc/handling-metadata-with-the-jdbc-driver.md)
* [Con la copia Bulk con il Driver JDBC](../../connect/jdbc/using-bulk-copy-with-the-jdbc-driver.md)
* [Utilizzo di Always Encrypted con il JDBC Driver](../../connect/jdbc/using-always-encrypted-with-the-jdbc-driver.md)
* [Utilizzo di parametri con valori di tabella](../../connect/jdbc/using-table-valued-parameters.md)
* [Caratteristiche internazionali del Driver JDBC](../../connect/jdbc/international-features-of-the-jdbc-driver.md)
* [Riferimento all'API del Driver JDBC](../../connect/jdbc/reference/jdbc-driver-api-reference.md)

