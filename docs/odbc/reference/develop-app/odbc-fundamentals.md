---
title: Nozioni fondamentali su ODBC | Documenti Microsoft
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.reviewer: 
ms.suite: 
ms.technology:
- drivers
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: bdaa40d5-1c63-4aee-94fb-7b2edd0bdc47
caps.latest.revision: 5
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: bf03977c62729c273b66d2ab4af8cae9e239dfd9
ms.contentlocale: it-it
ms.lasthandoff: 09/09/2017

---
# <a name="odbc-fundamentals"></a>Nozioni fondamentali su ODBC
In questa sezione vengono trattati gli argomenti seguenti.  
  
-   [Handle](../../../odbc/reference/develop-app/handles.md)  
  
-   [Buffer](../../../odbc/reference/develop-app/buffers.md)  
  
-   [Tipi di dati ODBC](../../../odbc/reference/develop-app/data-types-in-odbc.md)  
  
-   [Livelli di conformità](../../../odbc/reference/develop-app/conformance-levels.md)  
  
-   [Ambiente di connessione e gli attributi di istruzione](../../../odbc/reference/develop-app/environment-connection-and-statement-attributes.md)  
  
-   [Tabelle e viste](../../../odbc/reference/develop-app/tables-and-views.md)
