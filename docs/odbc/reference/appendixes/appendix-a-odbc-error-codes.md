 ...
 **SQLGetDiagRec** oppure **SQLGetDiagField** restituisce i valori SQLSTATE secondo quanto definito da Open Group *gestione dati: Structured Query Language (SQL), versione 2* (marzo 1995). I valori SQLSTATE sono stringhe che contengono da cinque caratteri. Nella tabella seguente sono elencati i valori SQLSTATE in grado di restituire un driver per **SQLGetDiagRec**.  
