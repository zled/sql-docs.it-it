---
title: Riferimenti al linguaggio Visual FoxPro | Documenti Microsoft
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.reviewer: 
ms.suite: 
ms.technology:
- drivers
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- commands for FoxPro ODBC driver [ODBC]
- Visual FoxPro ODBC driver [ODBC], commands
ms.assetid: 8e052213-ca6f-44ca-917e-054ecb3eb0e5
caps.latest.revision: 5
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: a833509f772205258e10ec4b589da10ab2a1cbbe
ms.contentlocale: it-it
ms.lasthandoff: 09/09/2017

---
# <a name="visual-foxpro-language-reference"></a>Riferimenti al linguaggio Visual FoxPro
In questa sezione vengono trattati gli argomenti seguenti.  
  
-   [ALTER TABLE - comando SQL](../../odbc/microsoft/alter-table-sql-command.md)  
  
-   [Crea tabella - comando SQL](../../odbc/microsoft/create-table-sql-command.md)  
  
-   [DELETE - comando SQL](../../odbc/microsoft/delete-sql-command.md)  
  
-   [ELIMINARE i comandi di TAG](../../odbc/microsoft/delete-tag-command.md)  
  
-   [TRASCINARE il comando di tabella](../../odbc/microsoft/drop-table-command.md)  
  
-   [Comando indice](../../odbc/microsoft/index-command.md)  
  
-   [INSERT - comando SQL](../../odbc/microsoft/insert-sql-command.md)  
  
-   [Selezionare - comando SQL](../../odbc/microsoft/select-sql-command.md)  
  
-   [Comando ANSI SET](../../odbc/microsoft/set-ansi-command.md)  
  
-   [Comando BLOCKSIZE SET](../../odbc/microsoft/set-blocksize-command.md)  
  
-   [Comando COLLATE SET](../../odbc/microsoft/set-collate-command.md)  
  
-   [Comando DELETED SET](../../odbc/microsoft/set-deleted-command.md)  
  
-   [Comando esatto SET](../../odbc/microsoft/set-exact-command.md)  
  
-   [Comando esclusivo SET](../../odbc/microsoft/set-exclusive-command.md)  
  
-   [Comando NULL SET](../../odbc/microsoft/set-null-command.md)  
  
-   [Comando percorso SET](../../odbc/microsoft/set-path-command.md)  
  
-   [Comando RIELABORAZIONE SET](../../odbc/microsoft/set-reprocess-command.md)  
  
-   [Comando univoco SET](../../odbc/microsoft/set-unique-command.md)  
  
-   [Comando SQL UPDATE-](../../odbc/microsoft/update-sql-command.md)  
  
-   [Tipi di dati di campo di Visual FoxPro](../../odbc/microsoft/visual-foxpro-field-data-types.md)
