...
manager: "erikre"
---
# Configurare l&#39;accesso HTTP ad Analysis Services in Internet Information Services (IIS) 8.0
...
##  <a name="bkmk_overview"></a> Panoramica  
 ...
 ![Diagramma in cui sono mostrate le connessioni tra componenti](../../analysis-services/instances/media/ssas.gif "Diagramma in cui sono mostrate le connessioni tra componenti")  
...
1.  Copiare i file seguenti, disponibili in \<unità>:\Programmi\Microsoft SQL Server\\<istanza\>\OLAP\bin\isapi: MSMDPUMP.DLL, MSMDPUMP.INI, e una cartella Risorse.  
...
2.  Nel server Web creare una cartella nuova: \<unità>:\inetpub\wwwroot\\**OLAP**  
...
2.  Aprire la cartella del server, fare clic con il pulsante destro del mouse su **Pool di applicazioni** e scegliere **Aggiungi pool di applicazioni**. Creare un pool di applicazioni con il nome **OLAP**, usando .NET Framework, con Modalità pipeline gestita impostata su **Classica**.  
     ...
     ![Schermata della finestra di dialogo Aggiungi pool di applicazioni](../../analysis-services/instances/media/ssas-httpaccess.png "Schermata della finestra di dialogo Aggiungi pool di applicazioni")  
...
3.  Per impostazione predefinita, IIS crea pool di applicazioni usando **ApplicationPoolIdentity** come identità di sicurezza,una scelta valida per l'accesso HTTP ad Analysis Services. Se si hanno motivi specifici per modificare l'identità, fare clic con il pulsante destro su **OLAP** e scegliere **Impostazioni avanzate**. Selezionare **ApplicationPoolIdentity**. Fare clic sul pulsante **Cambia** di questa proprietà per sostituire l'account predefinito con quello personalizzato che si vuole usare.  
     ...
     ![Schermata della pagina delle proprietà Impostazioni avanzate](../../analysis-services/instances/media/ssas-httpaccess-advsettings.png "Schermata della pagina delle proprietà Impostazioni avanzate")  
...
2.  Fare doppio clic su **Autenticazione** nella sezione IIS della pagina principale.  
     ...
     ![Schermata della pagina principale di Gestione IIS](../../analysis-services/instances/media/ssas-httpaccess-iis.png "Schermata della pagina principale di Gestione IIS")  
  
3.  Se si usa la sicurezza integrata di Windows, abilitare **Autenticazione Windows**.  
  
     ![Schermata delle impostazioni dell'autenticazione della directory virtuale](../../analysis-services/instances/media/ssas-httpaccess-iisauth.png "Schermata delle impostazioni dell'autenticazione della directory virtuale")  
...
7.  Fare clic con il pulsante destro del mouse in un punto qualsiasi della pagina e selezionare **Aggiungi mapping di script**. Nella finestra di dialogo Aggiungi mapping di script specificare **\*.dll** come percorso della richiesta e c:\inetpub\wwwroot\OLAP\msmdpump.dll come file eseguibile e digitare **OLAP** come nome. Mantenere tutte le restrizioni predefinite associate a questo mapping di script.  
     ...
     ![Schermata della finestra di dialogo Aggiungi mapping di script](../../analysis-services/instances/media/ssas-httpaccess-addscript.png "Schermata della finestra di dialogo Aggiungi mapping di script")  
...
```  
 ...
 Se è stata configurata un'istanza denominata o predefinita di Analysis Services in modo da essere in ascolto su una porta fissa, è necessario aggiungere al nome del server il numero di porta (ad esempio \<ServerName>AW-SRV01:55555\</ServerName>) e consentire connessioni in ingresso per la porta in questione in Windows Firewall.  
 ...
 **Risolvere i problemi delle connessioni con Internet Explorer**  
 ...
 Quando una richiesta di connessione termina con l'errore 'Impossibile stabilire una connessione al server '\<nome server>' oppure 'Analysis Services non è in esecuzione nel computer specificato, può essere impossibile proseguire.  
...
2.  Ritentare la connessione, ad esempio http://my-web-srv01:8080/OLAP/msmdpump.dll.  
 ...
 **Verificare le connessioni tramite AMO**  
 ...
 Per altre informazioni sulla connessione a livello di programmazione, vedere [Implementazione di connessioni protette in ADOMD.NET](../Topic/Establishing%20Secure%20Connections%20in%20ADOMD.NET.md).  
