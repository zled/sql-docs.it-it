...
ms.topic: "article"
helpviewer_keywords: 
  - "Generazione guidata schema, schema del database"
  - "schema di database [Analysis Services]"
  - "schema relazionale [Analysis Services], schema del database"
  - "opzioni schema area di interesse [Analysis Services]"
  - "opzioni schema area di gestione temporanea [Analysis Services]"
...
## Conversione del tipo di dati e lunghezze predefinite  
 La Generazione guidata schema ignora sempre i tipi di dati eccetto che per le colonne che usano il tipo di dati **wchar** di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. La dimensione dei dati **wchar** viene convertita direttamente nel tipo di dati **nvarchar**. Tuttavia, se la lunghezza specificata di una colonna con il tipo di dati **wchar** è maggiore di 4000 byte, la Generazione guidata schema visualizzerà un errore.  
