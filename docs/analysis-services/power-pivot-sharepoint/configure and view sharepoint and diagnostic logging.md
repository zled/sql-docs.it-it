---
title: "Configurare e visualizzare i file di log di SharePoint e la registrazione diagnostica (Power Pivot per SharePoint) | Microsoft Docs"
ms.custom: ""
ms.date: "12/09/2016"
...
manager: "erikre"
---
# Configurare e visualizzare i file di log di SharePoint e la registrazione diagnostica (Power Pivot per SharePoint)
...
3.  Scorrere fino a **Servizio [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)]**.  
  
4.  Espandere la categoria e selezionare le singole categorie.  
  
     **Richiesta pagine applicazione** consente di specificare gli eventi attivati dall'applicazione di servizio durante l'individuazione di un'istanza di [!INCLUDE[ssGeminiSrv](../../includes/ssgeminisrv-md.md)] per il caricamento di un'origine dati [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] e la comunicazione con altri server nella farm.  
  
     **Elaborazione richieste[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] consente di specificare gli eventi attivati da richieste di query su un database ** caricato in un server della farm.  
  
     **Utilizzo[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] consente di specificare un evento correlato alla raccolta di dati di utilizzo di **.  
...
-   Cercare lo stato di errore "Elevato" o "Eccezione". Cercare "Servizio [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)]".  
...
#### Usare un visualizzatore di log  
...
#### Voci per i servizi Power Pivot  
...
|-------------|----------|--------------|-----------|-------------|-------------|  
|w3wp.exe|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Servizio|Utilizzo|Verbose|Non sono disponibili statistiche sulle richieste, nulla da registrare.|A intervalli predefiniti, il servizio riporta le statistiche sulle risposte alle query come evento di utilizzo nel sistema di raccolta dei dati di utilizzo. Questo messaggio indica che non ci sono statistiche sulle query da riportare.|  
|w3wp.exe|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Servizio|Front-end Web|Verbose|Avvio individuazione server applicazioni per l'origine dati=\<*percorso*>|Quando riceve una richiesta di connessione, il servizio [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] identifica un [!INCLUDE[ssGeminiSrv](../../includes/ssgeminisrv-md.md)] disponibile per gestire la richiesta. Se è presente un solo server nella farm, il server locale accetta la richiesta in tutti i casi.|  
|w3wp.exe|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Servizio|Front-end Web|Dettagliato|Individuazione del server applicazioni riuscita.|La richiesta è stata allocata a un'applicazione di servizio [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] .|  
|w3wp.exe|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Servizio|Front-end Web|Verbose|Reindirizzamento della richiesta per l'\<*origine dati PowerPivot*> a [!INCLUDE[ssGeminiSrv](../../includes/ssgeminisrv-md.md)].|La richiesta è stata inoltrata a [!INCLUDE[ssGeminiSrv](../../includes/ssgeminisrv-md.md)].|  
|w3wp.exe|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Servizio|Elaborazione delle richieste|Verbose|Reindirizzamento della richiesta per NomeUtente\<*utente SharePoint*> al database|Una connessione rappresentata all'origine dati [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] è stata creata per conto dell'utente SharePoint.|  
