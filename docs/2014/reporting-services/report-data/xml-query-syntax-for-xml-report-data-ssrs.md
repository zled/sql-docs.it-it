...
>  Quando uno spazio dei nomi del servizio Web contiene una barra (`/)`, includere sia il metodo del servizio Web che l'azione SOAP, per consentire all'estensione per l'elaborazione dati XML di derivare lo spazio dei nomi in modo corretto.  
 ...
 Per un documento XML incorporato, il codice XML `Query` definisce i dati XML incorporati da utilizzare, sono inclusi gli spazi dei nomi facoltativo e contiene un valore XML facoltativo `ElementPath`...  
...
-   Per le richieste del servizio Web, i parametri di query vengono passati al metodo del servizio Web. Per definire un parametro di query, usare la pagina **Parametri** della finestra di dialogo **Proprietà set di dati** . Per altre informazioni, vedere [finestra di dialogo Proprietà set di dati, i parametri](dataset-properties-dialog-box-parameters.md).  
...
|---------------------|-------------------|  
|Dati XML del servizio Web dal metodo listchildren sono indicate.|`<Query>`<br /><br /> `<Method Name="ListChildren" Namespace="http://schemas.microsoft.com/sqlserver/2005/06/30/reporting/reportingservices" />`<br /><br /> `</Query>`|  
