 ...
 *ambito*  
 (`String`) Il nome di un set di dati, gruppo o area dati che contiene il report elementi a cui applicare la funzione di aggregazione. *Ambito* deve essere una costante di tipo stringa e non può essere un'espressione. Se si omette *scope* , viene usato l'ambito corrente.  
  
## <a name="return-type"></a>Tipo restituito  
 Il tipo restituito dipende dal provider di dati. Restituisce `Nothing` se il provider di dati non supporta questa funzione o dati non sono disponibili.  
...
## <a name="remarks"></a>Remarks  
 ...
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] rilevato l'utilizzo del `Aggregate` funzione nelle espressioni del report per determinare se visualizzare le aggregazioni server nelle righe di dettaglio. Se si includono `Aggregate` in un'espressione in un'area dati, le aggregazioni server possono trovarsi soltanto sul gruppo totale o complessivi totale righe, non nelle righe di dettaglio. Se si desidera visualizzare le aggregazioni server nelle righe di dettaglio, non utilizzare la funzione `Aggregate`.  
...
## <a name="comparing-the-aggregate-and-sum-functions"></a>Confronto tra le funzioni Aggregate e Sum  
 Il `Aggregate` è diversa dalle funzioni di aggregazione numeriche come `Sum` in quanto il `Aggregate` funzione restituisce un valore che viene calcolato dall'estensione dei dati del provider o l'elaborazione dati. Funzioni di aggregazione numeriche come `Sum` restituiscono un valore che viene calcolato dall'elaboratore di report in un set di dati dal set di dati è determinato dal *ambito* parametro. Per altre informazioni, vedere le funzioni di aggregazione elencate in [Riferimento a funzioni di aggregazione &#40;Generatore report e SSRS&#41;](report-builder-functions-aggregate-functions-reference.md).  
