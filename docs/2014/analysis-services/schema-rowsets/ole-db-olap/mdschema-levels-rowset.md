---
title: Set di righe MDSCHEMA_LEVELS | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
|`CUSTOM_ROLLUP_SETTINGS`|`DBTYPE_I4`||Bitmap che specifica le opzioni di rollup personalizzato:<br /><br /> -   `MDLEVELS_CUSTOM_ROLLUP_EXPRESSION` (`0x01`) indica un'espressione non esiste per questo livello. (Deprecato)<br />-   `MDLEVELS_CUSTOM_ROLLUP_COLUMN` (`0x02`) indica che è presente una colonna di rollup personalizzato per questo livello.<br />-   `MDLEVELS_SKIPPED_LEVELS` (`0x04`) indica che è presente un livello ignorato associato a membri di questo livello.<br />-   `MDLEVELS_CUSTOM_MEMBER_PROPERTIES` (`0x08`) indica che i membri del livello dispongono di proprietà personalizzate dei membri.<br />-   `MDLEVELS_UNARY_OPERATOR` (`0x10`) indica che i membri del livello dispongono di operatori unari.|  
