...
|-------------------|-----------------|  
|AggregationStorage|Tipo di archiviazione per le aggregazioni.<br /><br /> Si applica solo alla partizione. Se applicato a una dimensione, deve essere `Regular.`.|  
...
|OnlineMode|Momento in cui l'immagine MOLAP è disponibile.<br /><br /> Il valore può essere `Immediate` o `OnCacheComplete`.|  
