...
|**Inizializzate sul lato client**|Selezionare questa opzione per usare il codice XML per il comando Analysis (XMLA), `NotifyTableChange`, per identificare le modifiche apportate alle tabelle sottostanti per l'oggetto. Questa opzione viene in genere selezionata se si desidera utilizzare un processo di notifica basato sul client.|  
