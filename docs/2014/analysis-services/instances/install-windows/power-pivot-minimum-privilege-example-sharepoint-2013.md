---
title: Esempio di una configurazione con privilegi minimi per PowerPivot per SharePoint 2013 | Documenti Microsoft
ms.custom: ''
ms.date: 03/06/2017
 ...
 **SPAdmin** è un account di dominio usato per installare e configurare la farm. Si tratta dell'account utilizzato per eseguire la configurazione guidata SharePoint e lo strumento di configurazione PowerPivot per SharePoint 2013 **SPAdmin** account è l'unico account che richiede diritti di amministratore locale. Prima di eseguire lo strumento di configurazione PowerPivot, concedere le **SPAdmin** dei privilegi necessari per l'istanza di database di SQL Server in cui SharePoint crea i database di contenuto e la configurazione dell'account. Per configurare l'account SPAdmin in uno scenario con privilegi minimi, l'account deve essere un membro dei ruoli **securityadmin** e **dbcreator**.  
