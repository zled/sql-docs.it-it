...
|Errori di colonna nei set di righe di tabella|Non elencato nella specifica XML for Analysis 1.1.|Il [errore](xml-elements-properties/error-element-xmla.md) elemento viene usato dal [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] affinché segnalino gli errori per gli elementi delle colonne contenuti un [riga](xml-elements-properties/error-element-xmla.md) elemento.|  
