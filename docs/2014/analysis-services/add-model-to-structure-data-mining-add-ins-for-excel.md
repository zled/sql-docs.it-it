---
title: Aggiunta modello a struttura (componenti aggiuntivi Data Mining per Excel Data) | Documenti Microsoft
ms.custom: ''
ms.date: 12/29/2017
...
# <a name="add-model-to-structure-data-mining-add-ins-for-excel"></a>Aggiunta modello a struttura (componenti aggiuntivi Data mining per Excel)
 ...
 Se l'istanza di Analysis Services non contiene già i dati necessari, usare il [Create Mining Structure &#40;componenti aggiuntivi Data Mining di SQL Server&#41; ](create-mining-structure-sql-server-data-mining-add-ins.md) procedura guidata per configurare una struttura di data mining. In alternativa, è possibile avviare una delle modellazioni guidate, quindi aggiungere un nuovo modello alla struttura creata dalla procedura guidata.  
...
1.  Nel **Data Mining** sulla barra multifunzione, fare clic sulla freccia sotto **avanzate**, quindi selezionare **aggiunta modello a struttura**.  
  
2.  Nel **Seleziona struttura** finestra di dialogo scegliere la struttura che contiene i dati si desidera utilizzare e quindi fare clic su **successivo**.  
  
     **Suggerimento**: se non si conoscono la struttura di data mining contiene i dati necessari, usare il **Document Model** procedura guidata per visualizzare le colonne e le statistiche di base sui dati.  
...
4.  È consigliabile scegliere anche il **parametri** pulsante per aprire la **i parametri dell'algoritmo** finestra di dialogo casella e personalizzare i parametri dell'algoritmo. Questa opzione rappresenta la modalità più semplice per creare modelli di data mining personalizzati.  
  
5.  Scegliere **Avanti**.  
  
6.  Nel **Seleziona colonne** la finestra di dialogo, esaminare l'elenco di colonne, se necessario, modificare l'utilizzo delle colonne per uno dei valori seguenti:  
    ...
    -   **Key**. A ogni modello deve essere associata almeno una chiave. A seconda del tipo di modello, potrebbe essere inoltre necessario l'opzione per chiavi speciali aggiuntive, ad esempio il **SequenceKey** oppure **TimeKey**.  
  
    -   **Non utilizzare**. Indica che i dati non devono essere utilizzati nel modello, anche se presenti nella struttura.  
  
7.  Fare clic su Sfoglia **(...)**  pulsante per aprire la **imposta flag di modellazione colonna** finestra di dialogo.  
     ...
     Se le colonne della struttura non sono applicabili per il nuovo algoritmo, è possibile ignorarle selezionando **non utilizzano**.  
  
8.  Nel **imposta flag di modellazione colonna** finestra di dialogo, rivedere o impostare i flag di modellazione, se presente.  
     ...
     Fare clic su **OK** quando lo scopo di chiudere la finestra di dialogo.  
