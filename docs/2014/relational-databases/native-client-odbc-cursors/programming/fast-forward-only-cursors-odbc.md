...
-   [SQLGetData](../../native-client-odbc-api/sqlgetdata.md) non è supportata. Le colonne del set di risultati devono essere associate a variabili di programma.  
  
-   Quando viene rilevata la fine del cursore, questo viene chiuso automaticamente dal server. L'applicazione deve ancora chiamare [SQLCloseCursor](../../native-client-odbc-api/sqlclosecursor.md) oppure [SQLFreeStmt](../../native-client-odbc-api/sqlfreestmt.md)(SQL_CLOSE), ma il driver non deve inviare la richiesta di chiusura al server. In questo modo viene evitato un round trip del server nella rete.  
