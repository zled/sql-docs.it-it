...
## <a name="dealing-with-data-types"></a>Gestione dei tipi di dati  
 Se lo schema specifica il `image`, `binary`, o `varbinary` [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] tipo di dati (tramite `sql:datatype`) e non specifica un tipo di dati XML, l'updategram presuppone che il tipo di dati XML è `binary base 64`. Se i dati sono di tipo `bin.base`, è necessario specificare in modo esplicito il tipo (`dt:type=bin.base` o `type="xsd:hexBinary"`).  
...
```  
 ...
 Nell'updategram seguente viene utilizzato questo schema XSD per aggiungere un nuovo record di dettagli ordine (un  **\<OD >** elemento il  **\<dopo >** blocco) per ordine numero 43860. L'attributo `mapping-schema` viene utilizzato per specificare lo schema di mapping nell'updategram.  
...
```  
 ...
 Lo schema XSD in questo esempio include  **\<cliente >** e  **\<ordine >** gli elementi e si specifica una relazione padre-figlio tra i due elementi. Identifica  **\<ordine >** come elemento padre e  **\<cliente >** come elemento figlio.  
