...
# <a name="using-annotated-xsd-schemas-in-queries-sqlxml-40"></a>Utilizzo di schemi XSD con annotazioni in query (SQLXML 4.0)
 ...
 Il  **\<SQL: XPath-query >** elemento consente di specificare una query XPath sulla vista XML definita dallo schema con annotazioni. Lo schema con annotazioni in base a cui deve essere eseguita la query XPath viene identificato tramite il `mapping-schema` attributo del  **\<SQL: XPath-query >** elemento.  
...
```  
 ...
 È infine possibile creare e utilizzare lo script di test SQLXML 4.0 (Sqlxml4test.vbs) per eseguire la query come parte di un file di modello. Per altre informazioni, vedere [schemi XDR &#40;deprecato in SQLXML 4.0&#41;](annotated-xdr-schemas-deprecated-in-sqlxml-4-0.md).  
...
## <a name="using-inline-mapping-schemas"></a>Utilizzo di schemi di mapping inline  
 ...
 Un modello può includere più schemi inline. Per utilizzare uno schema inline incluso in un modello, specificare il **id** dell'attributo con un valore univoco sul  **\<xsd: schema >** elemento e quindi utilizzare **#idvalue**per fare riferimento allo schema inline. Il **id** attributo comportamento è identico per i **SQL: ID** ({urn: schemas-microsoft-com: XML-sql} id) utilizzato negli schemi XDR.  
...
```  
 ...
 Il modello specifica inoltre due query XPath. Ognuna del  **\<xpath-query >** elementi identifica in modo univoco lo schema di mapping specificando il `mapping-schema` attributo.  
  
 Quando si specifica uno schema inline nel modello, il `sql:is-mapping-schema` annotazione deve essere specificata anche nel  **\<xsd: schema >** elemento. `sql:is-mapping-schema` utilizza un valore booleano (0=false, 1=true). Uno schema inline con **sql: schema di mapping è = "1"** viene considerato come uno schema con annotazioni inline e non viene restituito nel documento XML.  
 ...
 Oltre a specificare il `mapping-schema` dell'attributo IsDefault sul  **\<SQL: XPath-query >** dell'elemento in un modello (quando è presente una query XPath) o nella  **\<updg: Sync >** elemento in un updategram, è possibile eseguire le operazioni seguenti:  
  
-   Specificare il `mapping-schema` dell'attributo IsDefault sul  **\<radice >** elemento (dichiarazione globale) nel modello. Questo schema di mapping diventa quindi lo schema predefinito che verrà utilizzato da tutti i nodi XPath e updategram che non dispongono di alcuna annotazione `mapping-schema` esplicita.  
  
-   Specificare l'attributo `mapping schema` utilizzando l'oggetto ADO `Command`.  
  
 Il `mapping-schema` attributo specificato nella  **\<xpath-query >** o  **\<updg: Sync >** elemento ha la precedenza più alta; ADO `Command` oggetto ha precedenza minore.  
