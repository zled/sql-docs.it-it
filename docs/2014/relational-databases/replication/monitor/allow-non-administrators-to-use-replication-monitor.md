...
####  <a name="Permissions"></a> Permissions  
 Per consentire a utenti non amministratori di utilizzare Monitoraggio replica, un membro del **sysadmin** deve aggiungere l'utente al database di distribuzione e assegnarlo al ruolo predefinito del server di `replmonitor` ruolo.  
...
6.  Selezionare il `replmonitor` casella di controllo il **l'appartenenza al ruolo del Database** griglia.  
...
1.  Nel database di distribuzione del server di distribuzione eseguire [sp_helpuser &#40;Transact-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-helpuser-transact-sql). Se l'utente non è elencato nel `UserName` nel set di risultati, l'utente deve essere concesso accesso al database di distribuzione mediante il [Crea utente &#40;Transact-SQL&#41; ](/sql/t-sql/statements/create-user-transact-sql) istruzione.  
...
3.  Se l'utente non appartiene ai `replmonitor` ruolo, eseguire [sp_addrolemember &#40;Transact-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql) nel server di distribuzione nel database di distribuzione. Specificare il valore `replmonitor` per **@rolename** e il nome dell'utente del database o il [!INCLUDE[msCoName](../../../includes/msconame-md.md)] accesso di Windows aggiunto per **@membername**.  
...
1.  Per verificare che l'utente appartiene ai `replmonitor` ruolo, eseguire [sp_helprolemember &#40;Transact-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sp-helprolemember-transact-sql) nel server di distribuzione nel database di distribuzione e specificare il valore `replmonitor` per **@rolename**. Se l'utente non è elencato in `MemberName` nel set di risultati, attualmente non appartiene al ruolo.  
  
2.  Se l'utente appartiene alla `replmonitor` ruolo, eseguire [sp_droprolemember &#40;Transact-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sp-droprolemember-transact-sql) nel server di distribuzione nel database di distribuzione. Specificare il valore `replmonitor` per **@rolename** e il nome dell'utente del database o l'account di accesso di Windows rimosso per **@membername**.  
