...
1.  Nel database di pubblicazione del server di pubblicazione eseguire [sp_changemergearticle](/sql/relational-databases/system-stored-procedures/sp-changemergearticle-transact-sql). Specificare **@publication**, **@article**, il valore `subset_filterclause` per **@property**, l'espressione che definisce il filtro con parametri per **@value** (senza includere `WHERE`) e il valore **1** sia per **@force_invalidate_snapshot** e **@force_reinit_subscription**.  
