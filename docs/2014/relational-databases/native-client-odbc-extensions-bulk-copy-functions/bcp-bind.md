...
```  
 ...
 Nel caso di esempio, il *pData* parametro verrà impostato per l'indirizzo di un'istanza dichiarata della struttura, l'indirizzo del BCPBOUNDINT *iIndicator* membro della struttura. Il *cbIndicator* parametro verrà impostato per la dimensione di un numero intero (sizeof e il *cbData* parametro verrebbe impostato nuovamente le dimensioni di un numero intero (sizeof. Per la copia bulk il valore di una riga nel server che contiene un valore NULL per la colonna associata, il valore dell'istanza del *iIndicator* membro deve essere impostato su SQL_NULL_DATA.  
 ...
 *cbData*  
 ...
 L'impostazione *cbData* su SQL_VARLEN_DATA indica che il sistema utilizzerà un carattere di terminazione di stringa o altro metodo, per determinare la lunghezza dei dati copiati.  
...
```  
 ...
 Un modulo nonterminated di questo esempio potrebbe indicare che 15 caratteri essere copiate dal *szName* variabile alla seconda colonna della tabella associata:  
...
```  
 ...
 Se il limite [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] colonna caratteri "wide", in cui viene eseguita alcuna conversione [bcp_sendrow](bcp-sendrow.md). Se la colonna di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] è un tipo di carattere MBCS, la conversione da carattere wide a carattere multibyte viene eseguita quando i dati vengono inviati a [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
  
 *cbTerm*  
 Numero dei byte presenti nel carattere di terminazione per la variabile di programma, se presente. Se è presente alcun carattere di terminazione per la variabile, impostare *cbTerm* su 0.  
...
-   Qualsiasi altra lunghezza a 8 byte valida viene trattata come una lunghezza dei dati normale.  
 ...
 La chiamata [bcp_columns](bcp-columns.md) quando si utilizza **bcp_bind** genera un errore.  
...
## <a name="bcpbind-support-for-enhanced-date-and-time-features"></a>Supporto di bcp_bind per le caratteristiche avanzate di data e ora  
 Per informazioni sui tipi utilizzati con il *eDataType* parametro per i tipi di data/ora, vedere [modifiche di copia Bulk per avanzate di data e ora tipi &#40;OLE DB e ODBC&#41;](../native-client-odbc-date-time/bulk-copy-changes-for-enhanced-date-and-time-types-ole-db-and-odbc.md).  
