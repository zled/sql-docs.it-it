---
title: Creazione di stime basate su serie temporali (esercitazione intermedia di Data Mining) | Documenti Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
1.  Nel **stima modello di Data Mining** scheda della finestra di progettazione di Data Mining, nel **modello di Data Mining** fare clic su **Seleziona modello**.  
  
2.  Nel **Seleziona modello di Data Mining** finestra di dialogo espandere la struttura Forecasting, selezionare il **Forecasting** modello dall'elenco e quindi fare clic su **OK**.  
  
3.  Ignorare il **Seleziona tabella/e di Input** casella.  
...
7.  Nel **campo** colonna, selezionare **PredictTimeSeries**.  
    ...
    > [!NOTE]  
    >  Con i modelli Time Series è inoltre possibile utilizzare la funzione `Predict`. Per impostazione predefinita, tuttavia, la funzione Predict crea una sola stima per ogni serie. Pertanto, per specificare più intervalli per la stima, è necessario utilizzare il **PredictTimeSeries** (funzione).  
  
8.  Nel **modello di Data Mining** riquadro, selezionare la colonna del modello di data mining **quantità.** Trascinare Amount il **criteri/argomento** casella per il **PredictTimeSeries** funzione aggiunta in precedenza.  
  
9. Fare clic sui **criteri/argomento** , quindi digitare una virgola, seguita da **5**, dopo il nome del campo.  
  
     Il testo nel **criteri/argomento** finestra dovrebbe ora visualizzare quanto segue:  
...
14. Fare clic sui **criteri/argomento** , quindi digitare una virgola, seguita da **5**, dopo il nome del campo.  
...
2.  Nel **salvare Query risultati di Data Mining** della finestra di dialogo per **origine dati**selezionare [!INCLUDE[ssSampleDBDWobject](../includes/sssampledbdwobject-md.md)]. Se si desidera salvare i dati in un database relazionale diverso, è inoltre possibile creare una nuova origine dati.  
  
3.  Nel **nome della tabella** colonna, tipo di nome, una password temporanea nuova tabella, ad esempio **stime di prova**.  
