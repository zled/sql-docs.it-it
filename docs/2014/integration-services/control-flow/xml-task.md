...
|**IgnoreNamespaces**|Valore che specifica se l'URI (Uniform Resource Identifier) dello spazio dei nomi di un elemento e i nomi dei suoi attributi devono essere confrontati. Se questa opzione è impostata su `true`, due elementi che hanno lo stesso nome locale e uno spazio dei nomi diversi verranno considerati identici.|  
