...
|OpenRowset|String|Quando è AccessMode `OpenRowset`, il nome della tabella o della vista a cui accede la destinazione Excel.|  
|OpenRowsetVariable|String|Quando è AccessMode `OpenRowset from Variable`, il nome della variabile che contiene il nome della tabella o della vista a cui accede la destinazione Excel.|  
|SqlCommand|String|Quando è AccessMode `SQL Command`, l'istruzione Transact-SQL che la destinazione Excel Usa per specificare le colonne di destinazione per i dati.|  
