---
title: Ordinati e non ordinato di espressioni (XQuery) | Documenti Microsoft
ms.custom: 
ms.date: 03/14/2017
ms.prod: sql-non-specified
ms.reviewer: 
ms.suite: 
ms.technology: database-engine
ms.tgt_pltfrm: 
ms.topic: language-reference
applies_to: SQL Server
dev_langs: XML
helpviewer_keywords:
- unordered expressions [XQuery]
- ordered expressions [XQuery]
- expressions [XQuery], ordered
- expressions [XQuery], unordered
ms.assetid: 6e7b3631-38d5-4375-b565-21d2e3f36ae0
caps.latest.revision: "13"
author: JennieHubbard
ms.author: jhubbard
manager: jhubbard
ms.workload: Inactive
ms.openlocfilehash: db51f8e41a3b8f186d542ebbb01687742b2faced
ms.sourcegitcommit: 9678eba3c2d3100cef408c69bcfe76df49803d63
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 11/09/2017
---
# <a name="ordered-and-unordered-expressions-xquery"></a>Espressioni ordinate e non ordinate (XQuery)
[!INCLUDE[tsql-appliesto-ss2012-xxxx-xxxx-xxx_md](../includes/tsql-appliesto-ss2012-xxxx-xxxx-xxx-md.md)]

  Per impostazione predefinita, la modalità di ordinamento per tutte le operazioni [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] è **ordinati**. Di conseguenza, le sequenze di nodi restituiti da espressioni di percorso e le espressioni FLWOR, senza il **Ordina** clausola, sono nell'ordine del documento.  
  
 Altro **ordinati** e **non ordinata** sintassi descritta nella specifica XQuery non è supportata.  
  
## <a name="see-also"></a>Vedere anche  
 [Espressioni XQuery](../xquery/xquery-expressions.md)   
 [Istruzione FLWOR e iterazione &#40; XQuery &#41;](../xquery/flwor-statement-and-iteration-xquery.md)   
 [Espressioni di percorso &#40; XQuery &#41;](../xquery/path-expressions-xquery.md)  
  
  
